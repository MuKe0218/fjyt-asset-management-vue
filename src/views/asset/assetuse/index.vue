<template>
    ...
    <el-table v-loading="loading" :data="useList">
        ...
        <el-table-column label="领用编号" align="center" width="150">
            <template slot-scope="scope">
                ...
                @click="getUseDetail(scope.row)">
            {{scope.row.useCode }}
            </el-link>
            </template>
        </el-table-column>
        <el-table-column label="领用资产" prop="assetNames" align="center" width="150"></el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="700px">
        <el-form ref="form" :model="form" :rules="rules" label-width="100px">
          <el-row>
            <el-col :span="12">
              <el-form-item label="申请人" prop="useUser">
                <el-input
            ...
            <el-col :span="12">
            <el-form-item label="申请时间" prop="useTime">
              <el-date-picker
...
export default{
  ...
  methods: {
    ...
    //       if (this.form.assetSelectList[i].assetCode === this.assetList[j].assetCode) {
    //         list.push(this.assetList[j])
    //         break
    //       }
    //     }
    //   }
    ...
    // 详情
    getUseDetail (row) {
      listUseById(row.id).then(response => {
    ...
    handleDelete (row) {
      this.$modal
        .confirm('是否确认删除"' + row.useUser + '"的领用数据项？')
        .then(function () {
          return delUse(row.id)
        })
        .then(() => {
          this.getList()
